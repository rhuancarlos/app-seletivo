...
/**
Renomeadas as funções do My_model original (insert, update, delete)
Documentação: https://github.com/avenirer/CodeIgniter-MY_Model#limitlimitoffset0
*/
...
class MY_Model extends CI_Model
{
    ...
    private $_select = '*';
    private $debug_ativo = false;

    public function __construct($databaseConnection, $table) {
        parent::__construct();
        $this->_database_connection = 'postgres';
        ...
        */
        ...
        $this->table2 = $table;
        $this->load->model('auditoria_m');
    }

    public function utilizar_debug($status) {
        $this->debug_ativo = $status;
    }

    public function imprimir_debug($db) {
        if (!$this->debug_ativo) return;
     ...
     */
    public function insert_orm($data = NULL)
    {
     ...
     */
    public function update_orm($data = NULL, $column_name_where = NULL, $escape = TRUE)
    {
        if(!isset($data) && $this->validated!=FALSE)
        {
            $data = $this->validated;
        ...
        if($multi === FALSE)
        {
            ...
            if(isset($column_name_where))
            {
                ...
                {
                    $column_value = $column_name_where;
            ...
            if($escape)
            {
                ...
                {
                    $this->auditoria_m->auditoria('atualizar', $this->table, $column_value, $data);
                    $this->_prep_after_write();
            ...
            $this->auditoria_m->auditoria('atualizar', $this->table, $column_value, $data, 1);
            return FALSE;
        }
        // else...
        else
        {
            ...
            foreach($data as $row)
            {
                ...
                if(is_array($column_name_where))
                {
                    $column_value = $column_name_where[1];
                    $this->_database->where($column_name_where[0], $column_value);
                }
                else
                {
                ...
                if($escape)
                {
                    ...
                    {
                        $this->auditoria_m->auditoria('atualizar', $this->table, $column_value, $row);
                        $rows++;
                    }
                }
                else
     ...
     */
    public function delete_orm($where = NULL)
    {
    ...
    public function on($connection_group = NULL)
    {
        if(isset($connection_group))
        {
            ...
            $this->_database = $this->databaseConnection;
        }
        return $this;
    }
    ...
    private function _build_sorter($data, $field, $order_by, $sort_by = 'DESC') {
        // $this->_add_itens_sorter($field, $order_by, $sort_by);
        usort($data, array(new SortCompareClosure($field, $order_by, $sort_by), 'compare') );
        // $this->_clear_itens_sorter();
        return $data;
    }

    // private function _add_itens_sorter($field, $order_by, $sort_by){
    //     $this->field = $field;
    //     $this->order_by = $order_by;
    //     $this->sort_by = $sort_by;
    // }

    // private function _clear_itens_sorter(){
    //     $this->field = null;
    //     $this->order_by = null;
    //     $this->sort_by = null;
    // }

    // private function _sort_build_sorter($a, $b){
    //     $array_a = isset($a[$this->field]) ? $this->object_to_array($a[$this->field]) : NULL;
    //                 $array_b = isset($b[$this->field]) ? $this->object_to_array($b[$this->field]) : NULL;
    //                 return strtoupper($this->sort_by) ==  "DESC" ?
    //                     ((isset($array_a[$this->order_by]) && isset($array_b[$this->order_by])) ? ($array_a[$this->order_by] < $array_b[$this->order_by]) : (!isset($array_a) ? 1 : -1))
    //                     : ((isset($array_a[$this->order_by]) && isset($array_b[$this->order_by])) ? ($array_a[$this->order_by] > $array_b[$this->order_by]) : (!isset($array_b) ? 1: -1));
    // }
    ...
    */
/* - - - - - - - - - - - - - Nosso Antigo MyModel - - - - - - - - - - -- - - - */
    public function getAll($options = array()) {
        if(isset($options['order_by'])){
            $this->databaseConnection->order_by($options['order_by']);
        }
        $query = $this->databaseConnection->get($this->table2);
        $this->imprimir_debug($this->databaseConnection);
          // $d = $query->result();
        return ($query && $query->num_rows() > 0) ? $query->result() : false ;
    }

    public function getAssoc($pk, $descricao, $options = array()) {
        $dados = $this->getAll($options);
        if (!$dados) return array();

        $retorno = array();
        foreach ($dados as $dado) {
            $retorno[$dado->{$pk}] = $dado->{$descricao};
        }
        return $retorno;
    }
        
    public function getAllLimite($limite = array()){
        if(!empty($limite)){
            $this->databaseConnection->limit($limite['inicial'], $limite['final']);   
        }
        $query = $this->databaseConnection->get($this->table2);
        return ($query && $query->num_rows() > 0) ? $query->result() : false ;
    }

    public function countAll($limite = array()){
        $this->databaseConnection->count_all($this->table2);
    }
    public function getWhere($arr = false) {
        ($arr) ? $this->databaseConnection->where($arr) : null ;
        return $this->getAll();
    }

    public function getWhereFk($fk, $key) {
        $this->databaseConnection->where($fk, $this->databaseConnection->escape_str($key));
        $query = $this->databaseConnection->get($this->table2);
        return ($query && $query->num_rows() > 0) ? $query->result() : false ;
    }

    public function insert($obj){
        $this->databaseConnection->trans_begin(); //Abre a transação
        $this->databaseConnection->insert($this->table2, $obj);
        if ($this->databaseConnection->trans_status() === FALSE){ 
            $this->databaseConnection->trans_rollback(); 
            $retorno = false;
            $this->auditoria_m->auditoria('cadastrar', $this->table2, null, $obj, 1);
        }else{
            $retorno = $this->databaseConnection->insert_id();  
            $this->databaseConnection->trans_commit();
            $this->auditoria_m->auditoria('cadastrar', $this->table2, $retorno, $obj);
        }   
        return $retorno;
    }

    public function insertNT($obj){
        $retorno = $this->databaseConnection->insert($this->table2, $obj);
        if (!$retorno){ 
            $this->auditoria_m->auditoria('cadastrar', $this->table2, null, $obj, 1);
        }else{
            $retorno = $this->databaseConnection->insert_id();  
    ...
    public function delete($pk, $key) {
        $this->databaseConnection->trans_begin(); //Abre a transação
        $this->databaseConnection->where($pk, $this->databaseConnection->escape_str($key));
        $this->databaseConnection->delete($this->table2);
        if ($this->databaseConnection->trans_status() === FALSE) {
            $this->databaseConnection->trans_rollback();
            $this->auditoria_m->auditoria('remover', $this->table2, $key, null, 1);
            return false;
        } else {
            $this->databaseConnection->trans_commit();
            $this->auditoria_m->auditoria('remover', $this->table2, $key, null);
            return true;
        }
    }
    
    public function deleteWhereFk($fk, $listaIds) {
        $this->databaseConnection->trans_begin(); //Abre a transação
        $this->databaseConnection->where_in($fk, $listaIds);
        $this->databaseConnection->delete($this->table2);
        if ($this->databaseConnection->trans_status() === FALSE) {
            $this->databaseConnection->trans_rollback();
            foreach ($listaIds as $id) {
                $this->auditoria_m->auditoria('remover', $this->table2, $id, null, 1);
            }
            return false;
        } else {
            $this->databaseConnection->trans_commit();
            foreach ($listaIds as $id) {
                $this->auditoria_m->auditoria('remover', $this->table2, $id, null);
            }
            return true;
        }
    }

    public function update($obj, $pk, $key) {
        $this->databaseConnection->trans_begin(); //Abre a transação
        $this->databaseConnection->where($pk, $this->databaseConnection->escape_str($key));
        $this->databaseConnection->update($this->table2, $obj);
        if ($this->databaseConnection->trans_status() === FALSE) {
            $this->databaseConnection->trans_rollback();
            $this->auditoria_m->auditoria('atualizar', $this->table2, $key, $obj, 1);
            return false;
        } else {
            $this->databaseConnection->trans_commit();
            $this->auditoria_m->auditoria('atualizar', $this->table2, $key, $obj);
            return true;
        }
    }

    public function updateNT($obj, $pk, $key) {
        $this->databaseConnection->where($pk, $this->databaseConnection->escape_str($key));
        $retorno = $this->databaseConnection->update($this->table2, $obj);
        if (!$retorno) {
            $this->auditoria_m->auditoria('atualizar', $this->table2, $key, $obj, 1);
        } else {
            $this->auditoria_m->auditoria('atualizar', $this->table2, $key, $obj);
    ...
    public function countElements($pk, $key) {
        $this->databaseConnection->where($pk, $this->databaseConnection->escape_str($key));
        $query = $this->databaseConnection->get($this->table2);
    ...
    public function abrirTransacao(){
        $this->databaseConnection->trans_begin();
    }
    ...
    public function roolbackTransacao(){
        $this->databaseConnection->trans_rollback();
    }
}

class SortCompareClosure {
    private $field, $order_by, $sort_by;

    function __construct( $field, $orber_by, $sort_by ) {
        $this->field = $field;
        $this->orber_by = $orber_by;
        $this->sort_by = $sort_by;
    }
