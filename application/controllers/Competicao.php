...
class Competicao extends MY_Controller {
	...
	public function __construct() {
		...
		$this->stringController = "competicao";
		$this->load->library("assets", array(
			'header' => array(
				'titulo_home' => "Competição",
				'titulo_pagina_cadastro' => "Controle de Competição", 
	...
	public function getListParticipant() {
		...
		$listagem = $this->layoutListagemGeralParticipantes($participantes, $this->layout_coluna_id, $search);
		if(!$listagem) {
	...
	private function layoutListagemGeralParticipantes($dados, $layout_id, $search) {
		$ci	=& get_instance();
	...
	public function iniciarCompeticao() {
		header('Content-type: application/json');

		/**
		 * QUANDO CLICAR EM INICIAR COMPETIÇÃO NA VIEW O SISTEMA DEVERÁ:
		 * 1 - APRESENTAR TRÊS CAMPOS (DATA DE INICIO E FIM,  UMA BREVE DESCRIÇÃO ex"Acampoder Extreme 2021" E JÁ INFORMAR OS LIDERES)
		 * 2 - NO BACKEND: DEVERÁ INSERIR ESTA COMPETIÇÃO NA TABELA CORESPONDENTE e mediante o parametro "ABRIR INSCRIÇÕES QUANDO INICAR COMPETIÇÃO = TRUE"
		 * 
		 */
		$this->verificarSituacaoCompeticao()
		if($verificacao->status) {
			$retorno = array('status' => true, 'mensagem' => 'Competição Iniciada');
		} else {
			$retorno = array('status' => false, 'mensagem' => 'Falta iniciar a competição');
		}	
		return 
		print json_encode(array('status' => true, 'content' => 5^10));
	}

	private function verificarSituacaoCompeticao($idCompeticao) {
		if(!$idCompeticao) { return false; }

			return $this->competicao_m->getById($idCompeticao);
	}
}
